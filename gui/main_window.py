...
class MainWindow(tk.Tk):
    ...
    def _refresh_dropdowns(self):
        """Give each panel the app list minus targets already taken by
        OTHER panels. 'none' (and blank) may repeat, so never filtered."""
        full = getattr(self, "_last_app_list", None)
        if full is None and self.detector:
            full = self.detector.get_dropdown_values()
        if full is None:
            return
        targets = [p.get_target().strip().lower() for p in self._slider_panels]
        for i, panel in enumerate(self._slider_panels):
            taken = {t for j, t in enumerate(targets)
                     if j != i and t and t != "none"}
            panel.set_dropdown_values(
                [v for v in full if v.lower() not in taken])

    def _update_panel_active_states(self):
        """Refresh dropdown filtering and each slider's ● active /
    ...
    def _on_slider_changed(self, panel):
        ...
        # Enforce one-app-per-slider: if this panel now duplicates another
        # panel's target, bounce it back to "none" ("none"/blank may repeat).
        new = panel.get_target().strip().lower()
        if new and new != "none":
            for other in self._slider_panels:
                if other is panel:
                    continue
                if other.get_target().strip().lower() == new:
                    panel.set_target("none")   # re-triggers this handler; "none" never duplicates
                    return
